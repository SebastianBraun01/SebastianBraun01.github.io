...
function vertLine(x: number, height: number) {
  if(height > canvas.height || x > canvas.width) console.log("Fehler: data zu groß! (vertLine();)");
...
function horiLine(y: number, length: number) {
  if(length > canvas.width || y > canvas.height) console.log("Fehler: data zu groß! (horiLine();)");
...
function drawRect(länge: number, phase: number) {
  const amplitude = 150;
  const T = 120;
